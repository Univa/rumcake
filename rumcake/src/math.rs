pub use libm::atan2f; // we prefer the libm implementation of atan2
pub use libm::sqrtf; // we prefer the libm implementation of sqrt

#[inline(always)]
pub fn scale(x: u8, d: u8) -> u8 {
    (((x as u16) * (1 + d as u16)) >> 8) as u8
}

const SIN_TABLE: [u8; 256] = [
...
// Adapted from cichlid, using a correct sin table instead:
// https://github.com/sfleischman105/cichlid/blob/cf123dfcb57c2f9abf01d9cf1ba0a031d4cd25ee/src/math/trig.rs#L84
#[inline(always)]
pub fn sin(theta: u8) -> u8 {
    unsafe { *SIN_TABLE.get_unchecked(theta as usize) }
}
